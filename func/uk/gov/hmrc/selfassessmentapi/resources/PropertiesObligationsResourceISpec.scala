...
class PropertiesObligationsResourceISpec extends IntegrationBaseSpec {
  ...
  private trait Test {
    ...
    def uri: String = s"/ni/${nino.nino}/uk-properties/obligations"
  ...
  "retrieveObligations" should {
    ...
    "return code 404 when attempting to retrieve obligations for a properties business that does not exist" when {
      "a valid request is received" in new Test {
    ...
    "return code 404 when property id does not exist" when {
      "a valid request is received" in new Test {

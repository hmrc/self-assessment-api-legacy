...
class AppContext @Inject()(
  ...
  lazy val selfAssessmentContextRoute: String = config.getString(s"${runMode.env}.contextPrefix").getOrElse("")
  lazy val desEnv: String = config.getString(s"${runMode.env}.microservice.services.des.env").getOrElse(throw new RuntimeException("desEnv is not configured"))
  lazy val desToken: String = config.getString(s"${runMode.env}.microservice.services.des.token").getOrElse(throw new RuntimeException("desEnv is not configured"))
  ...
  lazy val featureSwitch: Option[Configuration] = config.getConfig(s"${runMode.env}.feature-switch")
  lazy val auditEnabled: Boolean = config.getBoolean(s"auditing.enabled").getOrElse(true)
  lazy val authEnabled: Boolean = config.getBoolean(s"${runMode.env}.microservice.services.auth.enabled").getOrElse(true)
  lazy val sandboxMode: Boolean = config.getBoolean(s"sandbox-mode").getOrElse(false)
  lazy val mtdDate: String = config.getString(s"${runMode.env}.mtd-date").getOrElse(throw new RuntimeException("mtd-date is not configured"))
}

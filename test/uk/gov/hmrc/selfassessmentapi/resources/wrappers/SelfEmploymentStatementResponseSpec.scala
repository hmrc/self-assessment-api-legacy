  ...
  "SelfEmploymentsStatementResponse" when {
    "obligation details are returned with an open EOPS obligation" should {
      "wrap valid response" in {
        val response = SelfEmploymentStatementResponse(HttpResponse(200, desOpenObligationJson.toString()))
    ...
    "obligation details are returned with a fulfilled EOPS obligation" should {
      "wrap valid response" in {
        val response = SelfEmploymentStatementResponse(HttpResponse(200, desFulfilledObligationJson.toString()))
      ...
      "return None" in {
        val noEopsObligations =
          s"""
             ...
             |}
           """.stripMargin
        val response = SelfEmploymentStatementResponse(HttpResponse(200, Json.parse(noEopsObligations).toString()))
        val eops = response.retrieveEOPSObligation(identifier = nino)
      ...
      "return None" in {
        val badEops =
          s"""
             ...
             |}
          """.stripMargin
        val response = SelfEmploymentStatementResponse(HttpResponse(200, Json.parse(badEops).toString()))
        val eops = response.retrieveEOPSObligation(identifier = nino)
        eops.right.get shouldBe None
      }
    }
